...
const TeamsPanel: React.FC<TeamsPanelProps> = ({
  ...
  const teamsData = useMemo(() => {
    ...
    const convertedBigQueryTeams: TeamData[] = bigQueryTeams
      .filter(bqTeam => !bqTeam.bigQueryData?.dateDisbanded)
      .map(bqTeam => ({
        id: bqTeam.id,
    ...
    // Only show auto-generated teams if there are NO database teams at all
    const autoGeneratedTeams = convertedBigQueryTeams.length === 0
      ? Array.from(chapterTeams.values()).filter(team => team.organizers.length > 0)
      : [];
